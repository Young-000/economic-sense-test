...
describe('coinService', () => {
  ...
  describe('보상 상수', () => {
    it('should have correct reward values', () => {
      expect(COIN_REWARDS.GAME_COMPLETE).toBe(5);
      expect(COIN_REWARDS.HIGH_TIER).toBe(10);
      expect(COIN_REWARDS.REWARDED_AD).toBe(30);
      expect(COIN_REWARDS.SHARE_RESULT).toBe(5);
      expect(COIN_REWARDS.DAILY_LOGIN).toBe(10);
      expect(COIN_REWARDS.STREAK_3).toBe(5);
      expect(COIN_REWARDS.STREAK_7).toBe(5);
      expect(COIN_REWARDS.STREAK_14).toBe(10);
      expect(COIN_REWARDS.STREAK_30).toBe(15);
    });
  ...
  describe('게임 완료 보상', () => {
    it('should reward 5 coins per game', () => {
      rewardGameComplete();
      expect(getBalance()).toBe(5);
    });

    it('should reward high tier 10 coins', () => {
      rewardHighTier();
  ...
  describe('보상형 광고', () => {
    it('should reward 30 coins per ad', () => {
      rewardRewardedAd();
  ...
  describe('일일 출석', () => {
    it('should reward 10 coins on first login', () => {
  ...
  describe('일일 공유', () => {
    it('should reward 5 coins on first share', () => {
      rewardShareResult();
      expect(getBalance()).toBe(5);
    });
  ...
  describe('스트릭 보너스', () => {
    ...
    it('should reward 5 coins for 7-day streak', () => {
      const result = rewardStreakBonus(7);
      expect(result).not.toBeNull();
      expect(getBalance()).toBe(5);
    });

    it('should reward 10 coins for 14-day streak', () => {
      const result = rewardStreakBonus(14);
      expect(result).not.toBeNull();
      expect(getBalance()).toBe(10);
    });

    it('should reward 15 coins for 30-day streak', () => {
  ...
  describe('일일 시나리오 (v3.1 리밸런스: 광고 실단가 1.54원)', () => {
    /** 보상형 광고 1회 실수익 (2026-08-06 콘솔 실측 가중평균, 원) */
    const REVENUE_PER_REWARDED_AD_WON = 1.54;
    ...
    function playActiveDay(): void {
      rewardDailyLogin();      // +10
      rewardStreakBonus(7);    // +5
      rewardGameComplete();    // +5
      rewardGameComplete();    // +5
      rewardGameComplete();    // +5
      rewardHighTier();        // +10
      rewardRewardedAd();      // +30
    ...
    it('활발한 유저 하루 적립이 195코인이다', () => {
      playActiveDay();
      // 10 + 5 + 15 + 10 + 150 + 5 = 195
      expect(getBalance()).toBe(195);
    });
    ...
    it('미션 보상을 더해도 초기 며칠 적립이 285코인을 넘지 않는다', () => {
      playActiveDay();

      // 미션 달성: 18 + 18 + 24 + 30 = 90 (다양한 미션 조합)
      rewardMission(18, '개미 투자자 달성');
      rewardMission(18, '습관 투자자 달성');
      rewardMission(24, '큰손 투자자 달성');
      rewardMission(30, '슈퍼개미 달성');

      // 195 + 90 = 285
      expect(getBalance()).toBe(285);
    });
  });
});
