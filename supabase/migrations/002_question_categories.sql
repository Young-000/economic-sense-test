...
-- 1. 질문 타입 enum
CREATE TYPE question_type AS ENUM ('earning', 'spending');
...
-- 카테고리 시드 데이터
INSERT INTO question_categories (type, code, name_ko, emoji, description) VALUES
-- 벌기 (Earning)
...
-- 금액 범위 시드 데이터
INSERT INTO amount_ranges (category_id, size, min_amount, max_amount, typical_amount, label_ko)
...
-- 4. 질문 시나리오 테이블
CREATE TABLE question_scenarios (
  id SERIAL PRIMARY KEY,
  amount_range_id INTEGER NOT NULL REFERENCES amount_ranges(id),
...
-- 6. 인덱스 생성
CREATE INDEX idx_question_categories_type ON question_categories(type);
CREATE INDEX idx_question_categories_code ON question_categories(code);
CREATE INDEX idx_amount_ranges_category ON amount_ranges(category_id);
CREATE INDEX idx_question_scenarios_amount_range ON question_scenarios(amount_range_id);
CREATE INDEX idx_question_scenarios_active ON question_scenarios(is_active);
...
SELECT
  ...
  qs.is_active
FROM question_scenarios qs
JOIN amount_ranges ar ON qs.amount_range_id = ar.id
JOIN question_categories c ON ar.category_id = c.id
